...
static const string robot_name = "panda_arm_balance_bot";
...
int main() {
	...
	static const string world_file = string(BALANCE_BOT_FOLDER) + "/world_balance_bot.urdf";
...
void simulation(std::shared_ptr<SaiSimulation::SaiSimulation> sim) {
	...
	sim->addSimulatedForceSensor(robot_name, "end-effector", T_link_sensor,
	...
	while (fSimulationRunning) {
		...
		// sending the force and moment data to Redis
		Vector3d force = sim->getSensedForce(robot_name, "end-effector", false);
		redis_client.setEigen(FORCE_SENSOR_KEY, force);
		Vector3d moment = sim->getSensedMoment(robot_name, "end-effector", false);
